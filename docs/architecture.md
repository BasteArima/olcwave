...
| Backend       | `olcwave-api`                                 | FastAPI. Вся логика. Взаимодействует с Postgres, опционально с Remnawave и Docker socket.           |
...
| Remnawave     | (внешний)                                     | Опционально. Backend синхронизирует пользователей и проверяет подписки через Remnawave API.        |
...
### Режим с Remnawave (`RW_ENABLED=true`):
...
### Режим без Remnawave (`RW_ENABLED=false`):

```
1. Проверить локального пользователя по short_uuid.
      └─ нет  → 404, ничего не происходит.

2. Проверить трафик:
      └─ превышен → вернуть placeholder-конфигурацию "Traffic limit Exceeded", HTTP 403.

3. Далее то же, что и в режиме с Remnawave — запуск контейнеров, генерация конфигов, возврат подписки.
```

```
Client ──GET /sub/{uuid}──► Backend
                               │
                               ├─ check local user (Postgres)
                               ├─ check traffic (Postgres)
                               ├─ for each missing profile:
                               │      generate config → docker run
                               └─ return OLCBox bundle
...
Таким образом, при первом открытии пользователем своей ссылки его контейнеры запускаются по требованию. Последующие запросы используют уже запущенные контейнеры. Разница только в шаге валидации пользователя.
...
TrafficManager работает независимо от режима Remnawave — он всегда активен и учитывает трафик локальных пользователей.
