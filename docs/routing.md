...
3. Все существующие OLCRTC-контейнеры перезапускаются с параметром `UPSTREAM_SOCKS=host.docker.internal:10808`, который перенаправляет их исходящий трафик через XrayCore.
...
2. Все OLCRTC-контейнеры перезапускаются без `UPSTREAM_SOCKS` (прямое подключение).
...
Frontend выполняет клиентскую валидацию JSON перед сохранением:
...
* При перезапуске API routing-контейнер восстанавливается автоматически (если `xray_routing_enabled=true`), но OLCRTC-контейнеры не перезапускаются - для этого нужно подождать следующего обновления подписки или перезапустить их вручную.
